...
proc configure {args} {
    ...
    set d [dict create xs $xs static $static ldir $ldir idir $idir config $config]
    return $d
}
proc Hinstall {} { return "?destination? ?config? ?-xs <path>? ?-static? ?-dynamic?\n\tInstall all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-xs <path> = path to Crossroads I/O\n\t-static = link Crossroads I/O statically, default is to link dynamically\n\t-dynamic = link Crossroads I/O dynamically" }
proc _install {args} {
    global packages
	...
	if {[string length $xs]} {
	    lappend rcargs -L [file join $xs lib] -I [file join $xs include]
	} else {
...
proc Hdebug {} { return "?destination? ?config? ?-xs <path>? ?-static? ?-dynamic?\n\tInstall debug builds of all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-xs <path> = path to Crossroads I/O\n\t-static = link Crossroads I/O statically, default is to link dynamically\n\t-dynamic = link Crossroads I/O dynamically" }
proc _debug {args} {
    global packages
    set d [eval configure $args]
    dict with d {}
...
proc _gui {} {
    global INSTALLPATH XSPATH BUILDSTATIC
    package require Tk
#    package require widget::scrolledwindow
    ...
    entry  .ez -textvariable ::XSPATH
    button .bz -command [list Browse ::XSPATH] -text Browse
...
proc Install {} {
    global INSTALLPATH XSPATH NOTE
	...
	_install $INSTALLPATH -xs $XSPATH %s
...
proc Debug {} {
    global INSTALLPATH XSPATH
    foreach p {.ei .ez .bi .bz .bs .i .d} {
	$p configure -state disabled
    }

    set fail [catch [format {
	_debug $INSTALLPATH -xs $XSPATH %s
